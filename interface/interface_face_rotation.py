...
def run_avatar(img_path):
    img_primer_rgba, source_img, source_crop_pts, source_crop_pts_vt, source_crop_rect = face_process(img_path, out_size)
    ...
    while True:
        ...
        driving_pts = detect_face_mesh([frame[:, :, :3]])[0]
        if driving_pts.sum() == 0:
            mat_driving = np.eye(4)
            # blendshape_verts_driving = np.zeros_like(driving_pts)
        else:
            driving_pts = adjust_verts(driving_pts)
            mat_driving,pts_normalized_driving, face_pts_mean_driving = calc_face_mat(driving_pts[np.newaxis, :, :], face_pts_mean, fit = False)
            # blendshape_verts_driving = pts_normalized_driving[0] - face_pts_mean_driving
            # print(blendshape_verts_driving)
            mat_driving = mat_driving[0]
        euler_angles = rotation_matrix_to_euler_angles(mat_driving[:3, :3])
        rotationMatrix = eularAngle2Matrix(euler_angles)

        new_source_pts = source_crop_pts.copy()
        ...
        frame = np.concatenate([in0, out], axis=1)
